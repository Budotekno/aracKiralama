...
{
    public partial class FrmYeniAracEkle : Form
    {
        public FrmYeniAracEkle()
        {
            InitializeComponent();
        }
        ...
        private void kayitEkle()
        {
            ...
            {
                ClsVeriTabaniIslemleri.baglanti.Open();
                string sorgu = "Insert into arac (plaka, marka, model, kasaTipi, aracTuru, renk, modelYili, vites, yakit, km, durum)" + " " +
                    "Values (@plaka, @marka, @model, @kasaTipi, @aracTuru, @renk, @modelYili,@vites, @yakit, @km, @durum) ";

                OleDbCommand ekle = new OleDbCommand(sorgu, ClsVeriTabaniIslemleri.baglanti);

                string plaka = txtPlaka1.Text + " " + txtPlaka2.Text + " " + txtPlaka3.Text;

                ekle.Parameters.AddWithValue("@plaka", plaka);
                ekle.Parameters.AddWithValue("@marka", cmbMarka.Text);
                ekle.Parameters.AddWithValue("@model", cmbModel.Text);
                ekle.Parameters.AddWithValue("@kasaTipi", cmbKasaTipi.Text);
                ekle.Parameters.AddWithValue("@aracTuru", lblAracTuru.Text);
                ekle.Parameters.AddWithValue("@renk", cmbRenk.Text);
                ekle.Parameters.AddWithValue("@modelYili", txtModelYili.Text);
                ekle.Parameters.AddWithValue("@vites", cmbVites.Text);
                ekle.Parameters.AddWithValue("@yakit", cmbYakit.Text);
                ekle.Parameters.AddWithValue("@km", txtKm.Text);
                ekle.Parameters.AddWithValue("@durum", lblDurumu.Text);
                ...
                ptbArac.Image.Save(ClsResimIslemleri.resimKaydet("arac", plakaEkle));
            ...
            catch (Exception hata)
            {
                ...
                MessageBox.Show(hata.Message, "Araç Kayıt Ekleme Hata Penceresi");
            }
        ...
        private void FrmYeniAracEkle_Load(object sender, EventArgs e)
        {
            secinizYazisi();
            markaEkle();
